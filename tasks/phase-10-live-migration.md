...
curl -s -X POST -u bot:bot123 http://localhost:8080/api/v1/stopbuy
echo "New trade entries: BLOCKED"

# Cancel all open orders
curl -s -X POST -u bot:bot123 http://localhost:8080/api/v1/forceexit/all -H 'Content-Type: application/json' -d '{"ordertype": "market"}'
...
SECURITY
[ ] Futures API key IP-whitelisted to 140.245.17.121 only
[ ] No withdrawal permissions on API key
[ ] .env file NOT committed to git (verify: git status)
[ ] All passwords changed from defaults (bot:bot123 → unique password)
...
```bash
curl -s -u bot:bot123 http://localhost:8080/api/v1/profit | python3 -m json.tool | grep -E 'profit_all|trade_count|winning_trades|max_drawdown'
```
