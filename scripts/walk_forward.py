    ...
    try:
        with log_path.open("w") as logf:
            proc = subprocess.run(
                ...
                timeout=21600,  # 6h — 37-pair universe needs ~5.5h training + backtesting
                # (was 16200/4.5h — all folds timed out mid-training for 37 pairs, leaving
                # folds:[] in summary. fold_03 was in active backtesting when killed at 4.5h.)
            )
    finally:
