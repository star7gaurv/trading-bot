...
# Replace the entire LatestRun function
new_func = """function LatestRun({ data, error, loading, lastUpdated }) {
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-1">
                ...
                <div className="bg-elevated border border-border rounded px-3 py-2">
                  ...
                  >
                    {totalPnl != null ? formatUsdt(totalPnl) : "—"}
                  </div>
                </div>
                {data.target_folds && (
                  <div className="bg-elevated border border-border rounded px-3 py-2">
              ...
              <div className="pt-2">
                <FoldTable folds={summary.folds || []} targetFolds={data.target_folds} />
              </div>
            </>
          ) : (
...
pattern = re.compile(r"function LatestRun\(\{.*?return <div.*?</div>;.*?\}", re.DOTALL)
new_content = pattern.sub(new_func, content)
# if it didn't match cleanly, let's try a simpler split
if new_content == content:
    # Manual replace
    parts = content.split("function LatestRun({ data, error, loading, lastUpdated }) {")
    if len(parts) == 2:
        part2 = parts[1].split("export default function WalkForward() {")[0]
        new_content = parts[0] + new_func + "\n\n" + "export default function WalkForward() {" + parts[1].split("export default function WalkForward() {")[1]
